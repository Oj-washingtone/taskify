...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Platform, TextInput, Alert, KeyboardAvoidingView, FlatList, Modal, Animated, Keyboard, Switch, useWindowDimensions, LayoutAnimation, ActivityIndicator } from 'react-native';
...
export default function ScheduleScreen() {
      ...
      setTimeout(() => {
        if (timelineScrollRef.current) {
          ...
          if (isSameDay(selectedDate, new Date())) {
            targetHour = new Date().getHours(); // Always scroll to current time for today
          } else {
            const taskHours = tasksForSelectedDate.map(t => new Date(t.dueDate!).getHours());
            if (taskHours.length > 0) {
              targetHour = Math.min(...taskHours); // Scroll to the first task for future/past days
            }
          }

          if (targetHour !== null) {
            // Scroll to the target hour (minus 1 to give some top context)
  ...
  const sendEmailDigest = () => {
    ...
    // Uses native Mail app as real transactional staging fallback
    ...
    <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
            ...
            <TouchableOpacity onPress={() => router.push('/profile')}>
              <Ionicons name="person-circle-outline" size={32} color="#010F1C" />
            </TouchableOpacity>
          </View>
        </View>
      ...
      >
        <View style={{ paddingBottom: 100, paddingTop: 10, position: 'relative' }}>
          ...
          {timeSlots.map(hour => {
            // Find tasks for this specific hour, excluding all-day tasks
